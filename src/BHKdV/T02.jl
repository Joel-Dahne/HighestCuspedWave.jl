...
x / (π * u0(x) * log(10 + inv(x))) *
    ∫ abs(clausenc(x * (t - 1), -α) + clausenc(x * (1 + t), -α) - 2clausenc(x * t, -α)) * t * log(10 + inv(x * t)) dt
```
...
I₁ = gamma(1 + α) * sinpi(α / 2) * x^(-α - 1) *
    ∫ ((t - 1)^(-α - 1) + (1 + t)^(-α - 1) - 2t^(-α - 1)) * t * log(10 + inv(x * t)) dt
I₂ = ∫ (R(x * (1 - t)) + R(x * (1 + t)) - 2R(x * t)) * t * log(10 + inv(x * t)) dt
```
...
I₁₃ = gamma(1 + α) * sinpi(α / 2) * x^(-α - 1) *
    ∫ ((t - 1)^(-α - 1) + (1 + t)^(-α - 1) - 2t^(-α - 1)) * t * log(1 + 10x * t) dt
...
W(x) * I₁₃ = inv(log(x)) * inv(1 - x^p0) * sinpi(α / 2) *
    ∫ ((t - 1)^(-α - 1) + (1 + t)^(-α - 1) - 2t^(-α - 1)) * t * log(1 + 10x * t) dt
```
The factor `log(1 + 10x * t)` inside the integral can be enclosed
...
```
W(x) * I₁₃ = -inv(log(x)) * log(1 + 10x * [0, 1]) * W(x) * I₁₁
```
...
```
W(x) * I₂₃ = x^(1 + α) / (gamma(1 + α) * (1 - x^p0) * log(x)) *
    ∫ (R(x * (t - 1)) + R(x * (1 + t)) - 2R(x * t)) * t * log(1 + 10x * t) dt
```
We can compute an enclosure of `x^(1 + α) / (gamma(1 + α) * (1 - x^p0)
* log(x))` as above. Similarly to for `I₁₃` we can factor out an
enclosure of `log(1 + 10x * t)` from the integral, leaving us with the
