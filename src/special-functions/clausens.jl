...
ball, as determined by `iswide(x)`, it tries to compute a tighter
enclosure. If `s <= 1` then by [`lemma_clausens_monotone`](@ref) it is
monotone in `x` and it evaluates it at the endpoints. If we don't have
`s <= 1` then it first checks if the derivative doesn't contains zero,
if not it uses monotonicity to only evaluate at endpoints. If the
derivative does contain zero it uses a zeroth order approximation
instead.
...
is thus given by dividing these by `factorial(β)` and then composing
with `s`.
"""
